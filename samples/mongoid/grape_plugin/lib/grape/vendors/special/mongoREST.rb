    ...
    begin
      def self.generate_rest_call(hash)
        ...
        resource hash[:target_model].convert_model_name.to_sym do
          ...
          desc "read #{hash[:target_model].convert_model_name} record by: if no params #> return all,"+
                   " if _id given #> return that record,if parent_id given,return all child,"+
          ...
          desc "create #{hash[:target_model].convert_model_name} data by params, return boolean"
          params do
            hash[:target_model].properties.each do |field_name,type|
              optional field_name.to_sym, type: type ,desc: "field name ~> /#{field_name}/"
            end
          ...
          desc "update #{hash[:target_model].convert_model_name} data by params,return boolean"
          params do
